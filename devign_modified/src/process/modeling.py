...
def predict(step, test_loader_step,iter,fixed):
    import sys
    sys.stdout=open("/tf/devign/log_chatgpt","a",encoding="utf-8")
    print(f"Testing")
    with torch.no_grad():
        step.eval()
        stats = test_loader_step(step)
        #stats.outs().to_json("predict.json")
        file=f"out_chatgpt_51_{iter}.txt" if fixed else f"out_51_{iter}.txt"
        with open(file,"w") as fin:
            fin.write(str(stats.outs()))
        with open("label.txt","w") as fin:
            fin.write(str(stats.labels()))
