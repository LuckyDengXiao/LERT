...
def train_val_test_split(data_frame: pd.DataFrame,iter, shuffle=True,from_splitted=False):
    splitted_path="/tf/devign/data_splitted/"
    if from_splitted:
        print("from_splitted branch")
        train = load(splitted_path, f"train_{iter}.pkl")
        val = load(splitted_path, f"val_{iter}.pkl")
        test = load(splitted_path, f"test_{iter}.pkl")
        chatgpt = load(splitted_path, "chatgpt.pkl")
        train=train.append(chatgpt)
        val[["target","cwe"]].to_json(f"orgData_chat_gpt_51_{iter}.json")
    else:
        ...
        train.to_pickle(splitted_path + f"train_{iter}.pkl")
        val.to_pickle(splitted_path + f"val_{iter}.pkl")
        test.to_pickle(splitted_path + f"test_{iter}.pkl")
        val[["target","cwe"]].to_json(f"orgData_51_{iter}.json")
        #print(test.keys())
...
def loads(data_sets_dir, ratio=1):
    ff=[]
    fff=[]
    for f in listdir(data_sets_dir):
        if isfile(join(data_sets_dir, f)):
            if not "chatgpt" in join(data_sets_dir, f):
                ff.append(f)
            else:
                fff.append(f)
    data_sets_files = sorted(ff)
    if ratio < 1:
        data_sets_files = get_ratio(data_sets_files, ratio)
    #part chatgpt from dataset
    if not len(fff)==0:
        dataset2 = load(data_sets_dir, fff[0])
        fff.remove(fff[0])
        for ds_file in fff:
            dataset2 = dataset2.append(load(data_sets_dir, ds_file))
        dataset2.to_pickle("/tf/devign/data_splitted/chatgpt.pkl")
    ...
    for ds_file in data_sets_files:
        ...
        #dtset=load(data_sets_dir, ds_file)
        #print(dtset)
        #dataset = pd.concat([dataset,pd.DataFrame([dtset)])],ignore_index=True,join="outer")
        
        #dataset.loc[len(dataset)]=dtset
    df_one=dataset.loc[dataset["target"]==1]
    dfzero=dataset.loc[dataset["target"]==0].sample(n=df_one.shape[0]*5)
    #return dataset 
    return df_one.append(dfzero)
