...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="bigcode/starcoderbase-7b")
...
class SupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        self.user_tokens = self.tokenizer.encode(user_string, add_special_tokens=False)
        self.assistant_tokens = self.tokenizer.encode(assistant_string, add_special_tokens=False)
    ...
    def preprocess_one(self, example):
        ...
        for idx, message in enumerate(example["conversations"]):
            ...
            value = message["value"]
            if from_ == "human":
                value = self.user_string + value.strip() + "\n"
                ...
                value = value.strip() + self.tokenizer.eos_token + "\n"
                value_ids = self.tokenizer.encode(value, add_special_tokens=False)
                if len(input_ids) + len(self.assistant_tokens) + len(value_ids) > self.model_max_length:
                    break
                input_ids += self.assistant_tokens + value_ids
                labels += [self.ignore_index] * len(self.assistant_tokens) + value_ids
